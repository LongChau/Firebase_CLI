...
public static class StringExtensions
{
    ...
    */
    ...
    public static string FormatAmount(int amount, int maxAmount)
    {
        string ret = "";

        if (amount <= maxAmount)
            ret = "<color=\"green\">" + amount + "\\" + "<color=\"white\">" + maxAmount;
        else
            ret = "<color=\"red\">" + amount + "\\" + "<color=\"white\">" + maxAmount;
    ...
    public static string RandomString(string format)
    {
        ...
        foreach (var c in format)
        {
            switch (c)
            {
                case 'U':
                case 'u':
                    ret += RandomString(1, upper);
                    break;

                case 'L':
                case 'l':
                    ret += RandomString(1, lower);
                    break;

                case 'N':
                case 'n':
                    ret += RandomString(1, number);
                    break;
            }
        }
    ...
    private static string RandomString(int length, string source)
    {
        ...
        {
            ret += source[Random.Next(source.Length)];
        }
    ...
    public static bool Compare_NonSensitive(string s1, string s2)
    {
        string l1 = s1.ToLower();
        string l2 = s2.ToLower();
